...
public class BusinessDayManagementFacade {
    ...
    /**
     * @Description
     * 1. 영업일 변경(현재 영업일을 마감 상태로 변경 + 다음 영업일을 오픈 및 현재 영업일로 지정)
     * 2. 업데이트 내용을 객체에 반영
    ...
    /**
     * @Description
     * - 개인 마감 데이터 조회(거래내역과 마감 데이터)
     *      1. 마감 데이터(현금 입출금액, 전일자 현금 잔액이며 금일 마감 금액은 마감시 변경)
     *      2. 거래 내역 데이터(내역 상세 보기 시 조회 가능)
     *      3. 거래 내역의 입출금 총액( 거래내역 합산)
     */
     ...
     * @Description
     * - 개인 마감 데이터 조회(거래내역과 마감 데이터)
     *      1. 사원들의 마감 내역(전일자 현금 잔액, 입출금액, 금일 마감 금액 등)
     *      2. 지점의 전일자 현금 잔액
     *      3. 지점의 현금 입출금액, 금일 마감 금액(사원들의 )
     */
    ...
    public void closeByEmployee() {
        ...
        businessDayCloseService.employeeDeadlineStatusToClosed(currentBusinessDateAndEmployeeId);

    }

    /**
     * @Description
     * 1. 한명이라도 개인 마감 상태가 OPEN이라면 REQUIRED_EMPLOYEE_CLOSING
     * 2. 마감하려고 하는 지점의 마감상태가 CLOSED라면 ALREADY_CLOSED_BUSINESS_DAY
    ...
    public void closeByManager() {
        ...
        businessDayCloseService.branchDeadlineStatusToClosed(businessDateAndBranchId);
